...
///
/// Uses two Cubits:
/// - [PlayerCubit] for player profile and battle data
/// - [AiStrategyCubit] for AI analysis state
class ProfileScreen extends StatelessWidget {
            ...
            appBar: AppBar(
              ...
              actions: [
                if (playerState.isFromCache)
                  Tooltip(
                    message: 'Dados do cache (offline)',
                    child: Container(
                      ...
                      child: const Row(
                        ...
                        children: [
                          ...
                          SizedBox(width: 4),
                          Text('OFFLINE', style: TextStyle(fontSize: 9, color: Colors.orangeAccent, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ),
                IconButton(
                  ...
                  onPressed: () => context.read<PlayerCubit>().fetchPlayer(profile.tag),
                ),
              ],
            ),
            body: Column(
  ...
  Widget _buildAiAnalysisCard(BuildContext context, PlayerProfile profile, List<CrBattle> battleLog) {
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: BlocBuilder<AiStrategyCubit, AiStrategyState>(
                  buildWhen: (prev, curr) => curr is AiStrategyLoaded || prev is AiStrategyLoaded,
              ...
              BlocBuilder<AiStrategyCubit, AiStrategyState>(
                builder: (context, state) {
                  return switch (state) {
                    AiStrategyLoading() => const Padding(
                    ...
                    AiStrategyLoaded(:final report) => Padding(
                        padding: const EdgeInsets.all(16.0),
                    ...
                    AiStrategyError(:final failure) => Padding(
                        ...
                        child: ErrorDisplayWidget(
                          failure: failure,
                          onRetry: () => _triggerAnalysis(context, profile, battleLog),
                        ),
                      ),
                    AiStrategyInitial() => _buildInitialAnalysisUI(context, aiCubit, profile, battleLog),
                  };
                },
              ),
            ],
      ...
      child: Column(
        children: [
          ...
          Wrap(
            ...
            children: [
              _buildArchetypeChip(context, aiCubit, 'Aggressive (Beatdown)', Icons.bolt),
              _buildArchetypeChip(context, aiCubit, 'Defensive (Control)', Icons.shield),
              _buildArchetypeChip(context, aiCubit, 'Cycle (Fast)', Icons.loop),
              _buildArchetypeChip(context, aiCubit, 'Siege (Ranged)', Icons.castle),
              _buildArchetypeChip(context, aiCubit, 'Splash (Area)', Icons.waves),
            ],
          ),
          ...
          ElevatedButton.icon(
            onPressed: () => _triggerAnalysis(context, profile, battleLog),
  ...
  void _triggerAnalysis(BuildContext context, PlayerProfile profile, List<CrBattle> battleLog) {
    final adService = GetIt.instance<AdService>();
    final aiCubit = context.read<AiStrategyCubit>();

    if (kIsWeb) {
      // Skip ads on web
      aiCubit.generateStrategy(profile: profile, battles: battleLog);
    } else {
  ...
  Widget _buildArchetypeChip(BuildContext context, AiStrategyCubit cubit, String label, IconData icon) {
    final isSelected = cubit.selectedArchetype == label;
    return ChoiceChip(
      ...
      selected: isSelected,
      onSelected: (selected) {
        if (selected) cubit.setArchetype(label);
      },
